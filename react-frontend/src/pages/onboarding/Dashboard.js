...
import {
  ...
  summaryItemLabel,
  summaryPercent,
} from 'src/styles/js/onboarding/DashboardStyle';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboard = async () => {
      try {
        const res = await axiosInstance.get(PATH.API.ONBOARDING.DASHBOARD(empNo));
  ...
  if (!dashboard) {
    return <div style={dashboardPageStyle}>온보딩 대시보드를 불러오는 중입니다...</div>;
  }
